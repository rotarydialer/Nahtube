...
function isParentLoggedIn(req) {
    var roles = [];
    if (req.session.user) {
        console.log('Checking roles for %s:', req.session.user.username)
        if (req.session.user.roles) {
            roles = req.session.user.roles;
            if (roles.indexOf('parent') >= 0) {
                console.log(' --> Logged in as "%s" with role(s): %s', req.session.user.username, roles.join(', '));
                return true;
            }
        }     
    }
...
function checkLoginAndRedirect(req, res) {
    if (!isParentLoggedIn(req)) {
      ...
      return res.redirect('/parents/login' + refQstr);
    }
}
